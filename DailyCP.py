...
def autoFill(rows):
    """
    与其说其是再fill不如说其是在格式化表单，将将未选择的选项弹出
    :param rows: 已经根据本地表单内容填好的表单
    :return:
    """
    for item in rows:
        index = 0
...
def passwordEncrypt(text: str, key: str):
    """
            ...
            if os.path.exists(formpath):
                with open(formpath, "rb") as file:
                    def find(l, key_valueList: list):
                        """
                        :param l: 本地加载的已经填好的表单
                        :param key_valueList: 从接口得到的表单，的title和description键值对列表
                        :return:
                        """
                        for item in l:
                            # 这里的item就是本地表单中的一项
